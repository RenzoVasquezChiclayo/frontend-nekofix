...
import {
  adminDeleteProductGrade,
  adminListProductGrades,
} from "@/services/admin/product-grades.service";
import { useAdminAuth } from "@/store/admin-auth-context";
import { AdminHeader } from "@/components/admin/Header";
import { ProductGradeModal } from "@/components/admin/ProductGradeModal";
...
export function ProductGradesAdminView() {
  const { accessToken } = useAdminAuth();
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [rows, setRows] = useState<ProductGradeCatalog[]>([]);
  ...
  const [editing, setEditing] = useState<ProductGradeCatalog | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await adminListProductGrades(accessToken, {
        page,
        limit: PAGE_SIZE,
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await adminDeleteProductGrade(accessToken, deleteId);
      setDeleteId(null);
      notifySuccess("Grado eliminado correctamente");
      await load();
    } catch (e) {
      setError(getApiErrorMessage(e));
      notifyApiError(e);
  ...
  return (
    <>
      <AdminHeader
        title="Grados"
        description="Grados cosméticos por tipo de catálogo."
        actions={
          <button
            type="button"
            onClick={() => {
      ...
      <ConfirmModal
        ...
        description="¿Eliminar este grado? Los productos asociados pueden quedar inconsistentes."
