...
import {
  adminCreateProductGrade,
  adminUpdateProductGrade,
...
type Props = {
  open: boolean;
  accessToken: string;
  grade: ProductGradeCatalog | null;
  onClose: () => void;
  onSaved: () => void;
};

export function ProductGradeModal({ open, accessToken, grade, onClose, onSaved }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (grade) {
        await adminUpdateProductGrade(accessToken, grade.id, body);
        notifySuccess("Grado actualizado correctamente");
      } else {
        await adminCreateProductGrade(accessToken, body);
        notifySuccess("Grado creado correctamente");
      }
      onSaved();
      onClose();
    } catch (err) {
      if (err instanceof ApiError && err.status === 404) {
        setError(ADMIN_PRODUCT_GRADE_NOT_FOUND_MESSAGE);
        notifyError(ADMIN_PRODUCT_GRADE_NOT_FOUND_MESSAGE);
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <h2 className="text-lg font-semibold text-primary-950">
          {grade ? "Editar grado" : "Nuevo grado"}
        </h2>
